...
NrRepetitions             = 3000;                    % Number of Monte Carlo repetition (different channel realizations)             
...
PilotToDataPowerOffset    = 1:3:15;
Nrlength = length(PilotToDataPowerOffset);
% Channel
...
tic
for i_pd = 1:length(PilotToDataPowerOffset)

PilotMapping_OFDM = zeros(numel(PilotMatrix_OFDM),numel(PilotMatrix_OFDM));
PilotMapping_OFDM(PilotMatrix_OFDM(:)==1,1:NrPilotSymbols) = sqrt(PilotToDataPowerOffset(i_pd))*eye(NrPilotSymbols);
...
DataPowerReduction_OFDM = numel(PilotMatrix_OFDM)/(NrPilotSymbols*PilotToDataPowerOffset(i_pd)+NrDataSymbols_OFDM);

Rho_OFDM     = PilotToDataPowerOffset(i_pd)*DataPowerReduction_OFDM;
...
for i_rep = 1:NrRepetitions
    ...
    %% Channel H NMSE 
       NMSE_OFDM_LS(i_pd,i_rep)  = 10*log10((norm(abs(D_OFDM_est)-abs(D_OFDM)))^2/(norm(D_OFDM))^2);
       NMSE_OFDM_Imp(i_pd,i_rep) = 10*log10((norm(abs(H_Est_OFDM)-abs(D_OFDM)))^2/(norm(D_OFDM))^2);
       %% Channel_Temp h NMSE
       NMSE_OFDM_LS_Temp(i_pd,i_rep)    = 10*log10((norm(abs(h_est_OFDM)-abs(h_OFDM)))^2/(norm(h_OFDM))^2);
       NMSE_OFDM_Imp_Temp(i_pd,i_rep)   = 10*log10((norm(abs(h_est_OFDM_Temp)-abs(h_OFDM)))^2/(norm(h_OFDM))^2);
       NMSE_OFDM_Inter_Temp(i_pd,i_rep) = 10*log10((norm(abs(h_OFDM_Inter(:))-abs(h_OFDM)))^2/(norm(h_OFDM))^2);
   
end
TimeNeededSoFar = toc;
disp([int2str(i_pd/Nrlength*100) '% Completed! Time Left: ' int2str(TimeNeededSoFar/i_pd*(Nrlength-i_pd)/60) 'min, corresponding to approx. '  int2str(TimeNeededSoFar/i_pd*(Nrlength-i_pd)/3600) 'hour']);
end
%% Plot results
Markersize = 10; 
LineWidth  = 1.3;
figure(1);
hold off; 
plot(PilotToDataPowerOffset, mean(NMSE_OFDM_Inter_Temp,2),'--','Color' ,[1 0 0]*0.75,'LineWidth',LineWidth,'Markersize',Markersize);
hold on; grid on;
plot(PilotToDataPowerOffset, mean(NMSE_OFDM_LS,2),       '-*','Color' ,[1 0 1]*0.75,'LineWidth',LineWidth,'Markersize',Markersize);
plot(PilotToDataPowerOffset, mean(NMSE_OFDM_Imp,2),      '-o','Color' ,[0 1 0]*0.70,'LineWidth',LineWidth,'Markersize',Markersize);
plot(PilotToDataPowerOffset, mean(NMSE_OFDM_LS_Temp,2),  '-d','Color' ,[0 0 1]*0.80,'LineWidth',LineWidth,'Markersize',Markersize);
plot(PilotToDataPowerOffset, mean(NMSE_OFDM_Imp_Temp,2), '-.' ,'Color' ,[1 1 1]*0.00,'LineWidth',LineWidth,'Markersize',Markersize);
ylim([-35 -10]);

ylabel('Normalized Mean Square Error (dB)');
xlabel('Pilot-to-data-power-offset $\rho$','Interpreter','latex');
set(gca,"YTick",[-35 -30 -25 -20 -15 -10],'FontName','Times New Roman','FontSize',12,'GridLineStyle','--','GridLineWidth',1.2,'LooseInset', [0,0,0,0]);
