...
load("R_H_560.mat");
NMSE_For_Assign3  = [0.20658821, 0.10688727, 0.04050767, 0.01379575, 0.00484677, 0.00198538, 0.00118887, 0.00091326];
%The above data is provided by Heng Fu, who is the first author of article
% H. Fu, W. Si, and I.-M. Kim, "Deep Learning-based Joint Pilot Design and Channel 
% Estimation for OFDM Systems," IEEE Transactions on Communications, 2023.
%% Parameters
% Simulation
...
for i_rep = 1:NrRepetitions
    ...
    %% Channel H NMSE 
       NMSE_OFDM_LS(i_SNR,i_rep)  = 10*log10((norm(abs(D_OFDM_est)-abs(D_OFDM)))^2/(norm(D_OFDM))^2);
       NMSE_OFDM_Imp(i_SNR,i_rep) = 10*log10((norm(abs(H_Est_OFDM)-abs(D_OFDM)))^2/(norm(D_OFDM))^2);
       %% Channel_Temp h NMSE
       NMSE_OFDM_LS_Temp(i_SNR,i_rep)    = 10*log10((norm(abs(h_est_OFDM)-abs(h_OFDM)))^2/(norm(h_OFDM))^2);
       NMSE_OFDM_Imp_Temp(i_SNR,i_rep)   = 10*log10((norm(abs(h_est_OFDM_Temp)-abs(h_OFDM)))^2/(norm(h_OFDM))^2);
       NMSE_OFDM_Inter_Temp(i_SNR,i_rep) = 10*log10((norm(abs(h_OFDM_Inter(:))-abs(h_OFDM)))^2/(norm(h_OFDM))^2);
...
figure(1);
hold off; 
plot(M_SNR_dB, mean(NMSE_OFDM_Inter_Temp,2),'--','Color' ,[1 0 0]*0.75,'LineWidth',LineWidth,'Markersize',Markersize);
hold on; grid on;
plot(M_SNR_dB, mean(NMSE_OFDM_LS,2),       '- *','Color' ,[1 0 1]*0.75,'LineWidth',LineWidth,'Markersize',Markersize);
plot(M_SNR_dB, mean(NMSE_OFDM_Imp,2),      '- o','Color' ,[0 1 0]*0.75,'LineWidth',LineWidth,'Markersize',Markersize);
plot(M_SNR_dB, mean(NMSE_OFDM_LS_Temp,2),  '- d','Color' ,[0 0 1]*0.80,'LineWidth',LineWidth,'Markersize',Markersize);
plot(M_SNR_dB, 10*log10(NMSE_For_Assign3),   '-+','Color' ,[1 1 0]*0.65,'LineWidth',LineWidth,'Markersize',Markersize);
plot(M_SNR_dB, mean(NMSE_OFDM_Imp_Temp,2), '- ' ,'Color' ,[1 1 1]*0.10,'LineWidth',LineWidth,'Markersize',Markersize);
title('OFDM, 64-QAM, EVA, 500km/h@2.5GHz')
legend({'LS Linear interp $\hat{h}$','NIR extrap est. $\hat{\mathcal{H}}$','IR extrap est. $\hat{\mathcal{H}}$', ...
        'NIR extrap est. $\hat{h}$','JPDCE in [3] $\hat{h}$','IR extrap est. $\hat{h}$'},'Interpreter','latex','Location','southwest',NumColumns=2);
ylabel('Normalized Mean Square Error (dB)');
xlabel('Signal-to-Noise Ratio (dB)');
set(gca,'FontName','Times New Roman','FontSize',15,'GridLineStyle','--','GridLineWidth',1.2,'LooseInset', [0,0,0,0]);
